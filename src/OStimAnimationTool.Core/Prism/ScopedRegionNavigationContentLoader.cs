...
namespace OStimAnimationTool.Core.Prism
{
    ...
    public class ScopedRegionNavigationContentLoader : IRegionNavigationContentLoader
    {
        ...
        public object LoadContent(IRegion region, NavigationContext navigationContext)
        {
            ...
            var acceptingCandidates =
                candidates.Where(
                    v =>
                    {
                        if (v is INavigationAware navigationAware &&
                            !navigationAware.IsNavigationTarget(navigationContext)) return false;

                        if (!(v is FrameworkElement frameworkElement)) return true;
        ...
        protected virtual object CreateNewRegionItem(string candidateTargetContract)
        {
            ...
            catch (Exception e)
            {
                throw new InvalidOperationException(
                    string.Format(CultureInfo.CurrentCulture, "Cannot create navigation Target",
        ...
        /// <returns>
        ///     The candidate contract to seek within the <see cref="IRegion" /> and to use, if not found, when resolving from
        ///     the container.
        /// </returns>
        ...
        ///     <see cref="GetContractFromNavigationContext" />
        /// </param>
        /// <returns>An enumerable of candidate objects from the <see cref="IRegion" /></returns>
        protected virtual IEnumerable<object> GetCandidatesFromRegion(IRegion region,
            string candidateNavigationContract)
        {
            ...
            if (!contractCandidates.Any())
            {
                ...
                if (matchingType is null) return Array.Empty<object>();
